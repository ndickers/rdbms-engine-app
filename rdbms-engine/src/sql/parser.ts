...
import type { SelectCommand, SQLCommand } from "./types.js";


export function parseSQL(input: string): SQLCommand {
...
function parseInsert(sql: string): SQLCommand {
...
function parseSelect(sql: string): SQLCommand {

    const tableMatch = sql.match(/FROM (\w+)/i);
    if (!tableMatch) throw new Error("Invalid SELECT syntax");
...
function parseUpdate(sql: string): SQLCommand {

    const match = sql.match(
        /UPDATE (\w+) SET (\w+)\s*=\s*(.+) WHERE (\w+)\s*=\s*(.+)/i
    );
...
function parseDelete(sql: string): SQLCommand {

    const match = sql.match(
        /DELETE FROM (\w+) WHERE (\w+)\s*=\s*(.+)/i
    );
